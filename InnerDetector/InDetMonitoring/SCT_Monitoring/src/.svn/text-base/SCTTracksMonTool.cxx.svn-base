...
namespace {
  ...
  enum ParameterKey{ONE_D_LOCATION=1, TWO_D_LOCATION=3, TRACK_SEGMENT=11, OLD_PARAMETERS_VECTOR=31};
  ...
  etaRegion(const float eta){
    ...
    if (eta < -etaBoundary) return 0.0; //EC C
...
//====================================================================================================
//StatusCode SCTTracksMonTool::bookHistograms( bool /*isNewEventsBlock*/, bool isNewLumiBlock, bool isNewRun ) // hidetoshi 14.01.21 
StatusCode SCTTracksMonTool::bookHistograms()                                                                  // hidetoshi 14.01.21 
{
  ATH_MSG_DEBUG("SCTTracksMonTool::bookHistograms");
  m_path = (m_useIDGlobal) ? ("/InDetGlobal/") : ("");
...
StatusCode SCTTracksMonTool::fillHistograms(){
  ...
  if (m_doTrigger and  SCTMotherTrigMonTool::CheckTriggers() != StatusCode::SUCCESS ) ATH_MSG_WARNING("Triggers not found!");
  ...
  if (trkend == trkbegin) {
    if (m_doTrigger)
      for (int trig(0); trig != N_TRIGGER_TYPES ; ++trig) trackTriggerRate->Fill(trig,0);
  ...
  for(TrackCollection::const_iterator trkitr(tracks->begin()); trkitr!=trkend; ++trkitr){
    ...
    if(trkSum != NULL) scthits_on_trk = trkSum->get(Trk::numberOfSCTHits);
    else ATH_MSG_WARNING("TrackSummary not found not using track!");
          ...
          if (RawDataClus->detectorElement()->isSCT()) { 
            ...
            if (trkParameters){  
              ...
              if (!m_residualPullCalculator.empty()) { 
                const Trk::ResidualPull* residualPull(m_residualPullCalculator->residualPull(rio, trkParameters, m_doUnbiasedCalc ? Trk::ResidualPull::Unbiased : Trk::ResidualPull::Biased)); 
                if (!residualPull) { 
    ...
    for(unsigned int region(0); region != N_REGIONS; ++region){
      m_trackRate->Fill(float(region),double(hasHits[region])); //note: ordering was different in original code (was barrel, Eca, Ecb)
    }
  ...
  if(m_environment == AthenaMonManager::online){ 
    ...
    nTracks_pos++;
    if (nTracks_pos == m_evtsbins) nTracks_pos = 0;
...
//====================================================================================================
double SCTTracksMonTool::calculatePull(const double residual, const double trkErr, const double hitErr){
...
StatusCode SCTTracksMonTool::h1Factory(const std::string & name, const std::string & title, const float extent, MonGroup & registry, VecH1_t & storageVector){
