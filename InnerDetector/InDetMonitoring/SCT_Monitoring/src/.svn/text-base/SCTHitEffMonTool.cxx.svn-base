...
namespace{//anonymous namespace for functions at file scope
  ...
  double amgPseudoRapidity(const Amg::Vector3D & position){ 
    ...
    if (ma ==  0)  return  0; 
    if (ma ==  dz) return  std::numeric_limits<double>::max(); 
    if (ma == -dz) return  std::numeric_limits<double>::min(); 
...
StatusCode SCTHitEffMonTool::initialize(){
  ...
  if(m_chronotime) CHECK (service( "ChronoStatSvc", m_chrono));
...
StatusCode SCTHitEffMonTool::bookHistograms()
{
  ...
  if(newRun){
    ...
    for (Int_t isub(0) ; isub != N_REGIONS ; ++isub) {
      ...
      CHECK (bookEffHisto(m_Eff_summaryHistoFirstBCID[isub],     histGroupShift[isub], sumeffBCID[isub], sumefftitleBCID[isub], 2*n_layers[isub], 0., n_layers[isub]));
...
StatusCode SCTHitEffMonTool::fillHistograms(){
  ...
  if (evtStore()->contains<TrackCollection> ( m_TrackName) ){
    if (evtStore()->retrieve(m_tracks, m_TrackName).isFailure()) {
      ...
      return StatusCode::SUCCESS;
    }else VERBOSE ("Successfully retrieved " << m_TrackName << " : " << m_tracks->size() << " items");
  } else {
  ...
  for (TrackCollection::const_iterator itr(m_tracks->begin()); itr != endOfTracks; ++itr){
    ...
    bool useDetector[N_REGIONS] = {false, false, false};
    ...
    for (; TSOSItr != TSOSItrE; ++TSOSItr) {
      ...
      if (!surfaceID.is_valid()) continue;
      if ((*TSOSItr)->type(Trk::TrackStateOnSurface::Measurement) or (*TSOSItr)->type(Trk::TrackStateOnSurface::Outlier)) {
        if(m_pixelId->is_pixel(surfaceID)) m_pixelNHits++;
        if(m_trtId->is_trt(surfaceID)) m_trtNHits++;
      ...
      if ((*TSOSItr)->type(Trk::TrackStateOnSurface::Measurement)){ // Determine zmin and zmax taking multiple hits/side into account
        if (m_isCosmic){ // If this is cosmics use z, bad method but ok...
          ...
          }else WARNING("No track parameter found. Zmin and Zmax not recalculated.");
    ...
    for (TSOSItr = trackWithHoles->trackStateOnSurfaces()->begin(); TSOSItr != TSOSItrE; ++TSOSItr) {
      ...
      if (failCut(m_sctId->is_sct(surfaceID), "hit cut: is in SCT")) continue;

      Int_t isub(bec2Index(m_sctId->barrel_ec(surfaceID)));
      if (failCut(not (isub == INVALID_INDEX), "hit cut: valid sub-detector")) continue;
      ...
      Double_t phiUp(90.);
      Double_t theta(90.);
      if (trkParamOnSurface and not findAnglesToWaferSurface (trkParamOnSurface->momentum(), surfaceID, theta, phiUp)) WARNING ("Error from findAngles");
      ...
      if (m_useTRTPhase or m_isCosmic) {
        ...
        switch (eventID->run_number()) {
          case  91885: tmin= 1; tmax=40; break;
          case  91890: tmin= 1; tmax=40; break;
          case  92160: tmin=-10; tmax=30; break;
          case  96858: tmin=-10; tmax=20; break; //b field off
          case  96884: tmin=-10; tmax=20; break; //b field off
          case  96903: tmin=-5; tmax=20; break; //b field off
          case  96916: tmin=-10; tmax=15; break; //b field off
          case 108878: tmin=-99; tmax=99; break; //MC
        }
        if (failCut(timecor >= tmin and timecor <= tmax, "track cut: timing cut")) continue;
      ...
      if (m_isCosmic){
        ...
        layerSide = (m_sctId->barrel_ec(surfaceID) != 0) * N_BARRELS + float(m_sctId->layer_disk(surfaceID)) + (float(m_sctId->side(surfaceID)) == 0) * 0.5; 
        enclosingHits = layerSide > min_layerSide and layerSide < max_layerSide;
      }

      if (m_detailed) m_probEnclosedHisto[isub]->Fill(layerPlusHalfSide, Int_t(enclosingHits));

      if (m_requireEnclosingHits and not (layerPlusHalfSide == .5) and not (isub == 1 and layerPlusHalfSide == 3) and not (layerPlusHalfSide == 8))
        if (failCut(enclosingHits, "hit cut: enclosing hits")) continue;
      ...
      if (m_detailed) {
        ...
        if(ndf > 0) {
          ...
          if (otherFaceFound) m_Eff_chi2HistoFinal[isub]->Fill(chi2_div_ndf,m_eff);
      ...
      if (m_superDetailed) {
        ...
        if (ieta==0) m_xlResidualE0Histo[isub][layer]->Fill(xl, trackHitResidual);
        if (ieta==1) m_xlResidualE1Histo[isub][layer]->Fill(xl, trackHitResidual);
        if (m_unas) m_xlResidualUnasHisto[isub][layer]->Fill(xl, trackHitResidual);
      }
      // Check guard ring
      ...
      if (m_detailed) {
        if (m_eff == 1) {
          ...
          if (m_unas) m_localUnasHisto[isub]->Fill(xl,yl);
          m_localHitXHisto[isub]->Fill(xl);
          m_localHitYHistos[isub]->Fill(yl);
        } else m_localMissHisto[isub]->Fill(xl,yl);
      ...
      if (m_detailed) {
        ...
        if (m_unas == 1) m_ResidualUnasHisto[isub]->Fill(trackHitResidual);
        if (m_eff == 0)  m_ResidualMissHisto[isub]->Fill(trackHitResidual);
      ...
      if (m_superDetailed) {
        ...
        if (side == 0) {
          ...
          if (otherItr != mapOfTrackHitResiduals.end()) trackHitResidual2 = otherItr->second;
          m_TwoSidesResiduals[isub]->Fill(trackHitResidual, trackHitResidual2);
          if (m_eff == 0 or m_unas == 1) m_TwoSidesResidualsIneff[isub]->Fill(trackHitResidual, trackHitResidual2);
        }
      ...
      if (testOffline) INFO("Filling "<<histnumber<<", "<<side<<" eta "<<ieta<<" phi "<<iphi);
...
//StatusCode SCTHitEffMonTool::procHistograms(bool /*isEndOfEventsBlock*/, bool /*isEndOfLumiBlock*/, bool /*isEndOfRun*/){ // hidetoshi 14.01.22
StatusCode SCTHitEffMonTool::procHistograms(){                                                                              // hidetoshi 14.01.22
  if (m_superDetailed) {
      ...
      if (eff == 1){
        ...
        if (badChip != m_badChips->end()){
          ...
          for (Int_t i(0); i < 2 * N_CHIPS ; ++i){
            if ((status & (1 << i)) != 0) accSide[int(i >= N_CHIPS)] -= 1./N_CHIPS;
          }
...
StatusCode SCTHitEffMonTool::failCut(Bool_t value, std::string name){
  if (value){
    VERBOSE ("Passed " << name);
    return StatusCode::FAILURE;
  }
...
//StatusCode SCTHitEffMonTool::findAnglesToWaferSurface (const Trk::GlobalMomentum &mom, Identifier id, Double_t &theta, Double_t &phi){
StatusCode SCTHitEffMonTool::findAnglesToWaferSurface (const Amg::Vector3D &mom, Identifier id, Double_t &theta, Double_t &phi){ 
  phi = 90.;
  theta = 90.;
...
template <class T> StatusCode SCTHitEffMonTool::bookEffHisto(T*& histo, MonGroup & MG, TString name, TString title, Int_t nbin, Double_t x1, Double_t x2){
...
Double_t SCTHitEffMonTool::getResidual(const Identifier& surfaceID,const Trk::TrackParameters * trkParam,const InDet::SCT_ClusterContainer* p_sctclcontainer) {
  ...
  for( ; containerIterator != containerEnd; ++containerIterator){
    for (InDet::SCT_ClusterCollection::const_iterator rioIterator((*containerIterator)->begin()); rioIterator != (*containerIterator)->end(); ++rioIterator) {
