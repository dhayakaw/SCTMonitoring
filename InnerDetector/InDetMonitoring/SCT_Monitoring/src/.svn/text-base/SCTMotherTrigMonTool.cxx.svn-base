...
StatusCode SCTMotherTrigMonTool::initialize(){
  if ( ManagedMonitorToolBase::initialize().isFailure() ){
      msg(MSG::ERROR) << "Could not initialize Monitor tool base!" << endreq;
...
//---------------------------------------------------------
StatusCode SCTMotherTrigMonTool::checkTriggers(){
...
StatusCode SCTMotherTrigMonTool::printTriggers() const{
  for ( int i(0); i != N_TRIGGER_TYPES; ++i){
    if ( msgLvl(MSG::INFO) ) msg(MSG::INFO) << std::setw(7) << m_triggerNames[i] << " : " << m_firedTriggers.test(i) << endreq;
  }
  return StatusCode::SUCCESS;
}

bool SCTMotherTrigMonTool::hasTriggerFired(const unsigned int trigger) const{
  return ( (trigger< N_TRIGGER_TYPES) ? m_firedTriggers.test(trigger) : false);
}

bool SCTMotherTrigMonTool::isCalibrationNoise(const std::string & L1_Item) {
  ...
  msg(MSG::DEBUG) << "Trigger " << L1_Item << " = " << m_trigDec->isPassed(L1_Item) << endreq;

  return m_trigDec->isPassed(L1_Item);

}

bool SCTMotherTrigMonTool::isStream(const std::string & StreamName) {
  ...
  if ( evtStore()->contains<EventInfo>("ByteStreamEventInfo") ){
	...
	msg(MSG::DEBUG) << " i " << i << " Stream-Name " << evtInfo->trigger_info()->streamTags()[i].name()
			<< " type " << evtInfo->trigger_info()->streamTags()[i].type() << endreq;
  ...
  if (m_isStream == true)
    if ( msgLvl(MSG::DEBUG) ) 
      msg(MSG::DEBUG) << "Found " << StreamName << " Calibration Stream Event" << endreq;
